...
export class CategoryController {
  ...
  async searchCategoryByNameContaining(
    ...
    const totalCategories = await this.categoryService.totalCategory(
      keyword.trim(),
    );
    const foundCategories = await this.categoryService.searchAndSortCategories(
      keyword.trim(),
      sortType,
      page - 1,
      size,
  ...
  async deleteCategory(@Param('id') id: number) {
    const updatedCategory =
      await this.categoryService.updateCategoryIsDelete(id);
  ...
  async countTournamentByCategory(@Param('id') categoryId: number) {
    const total =
      await this.categoryService.countTournamentByCategory(categoryId);
